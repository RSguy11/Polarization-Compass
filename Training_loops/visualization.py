...
def create_training_plots(results, training_history, output_dir):
    ...
    ax5.bar(range(len(model_names)), test_mae, color='#9b59b6', alpha=0.8, edgecolor='black')
    ax5.axhline(y=5.0, color='green', linestyle='--', linewidth=2, alpha=0.7)
    ax5.set_ylabel('MAE (degrees)', fontweight='bold', fontsize=10)
    ax5.set_title('Test Set MAE (Held-out)', fontweight='bold', fontsize=11)
    ...
    for i, v in enumerate(test_mae):
        ax5.text(i, v, f'{v:.1f}°', ha='center', va='bottom', fontsize=8)
    
    ax6.bar(range(len(model_names)), cv_mae, color='#f39c12', alpha=0.8, edgecolor='black')
    ax6.axhline(y=5.0, color='green', linestyle='--', linewidth=2, alpha=0.7)
    ax6.set_ylabel('MAE (degrees)', fontweight='bold', fontsize=10)
    ax6.set_title('Cross-Validation MAE', fontweight='bold', fontsize=11)
    ax6.set_xticks(range(len(model_names)))
    ax6.set_xticklabels(model_names, rotation=15, ha='right', fontsize=9)
    ...
    width = 0.35
    ax7.bar(x_pos - width/2, train_mae, width, label='Train', color='#3498db', alpha=0.8, edgecolor='black')
    ax7.bar(x_pos + width/2, test_mae, width, label='Test', color='#9b59b6', alpha=0.8, edgecolor='black')
    ax7.axhline(y=5.0, color='green', linestyle='--', linewidth=2, alpha=0.7)
    ax7.set_ylabel('MAE (degrees)', fontweight='bold', fontsize=10)
    ax7.set_title('Train vs Test MAE', fontweight='bold', fontsize=11)
    ax7.set_xticks(x_pos)
    ax7.set_xticklabels(model_names, rotation=15, ha='right', fontsize=9)
